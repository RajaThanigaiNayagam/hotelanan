															...
															<td class="text-center">  <?php
																if($datechoosed == 'YES'){
																	...
																	foreach($Reservationdates as $Reservationdate){
																		...
																		where room.ID=:roomid and :ReservDate between booking.CheckinDate and DATEADD(day,-1,Convert( Date, booking.CheckoutDate ))"; //booking.CheckoutDate";  // date('Y-m-d', strtotime(booking.CheckoutDate .' -1 day'))"; //-------- Requêtes sql pour recuperer toutes les enregistrement depuis la table "roomcategory" -------//
																		$query1 = $dbh -> prepare($ret);
